...
void VehicleNode::publishPerceptionImage(Perception::ImageInfoType info, uint8_t *imageRawBuffer,
                       int bufferLen, void *userData) {
  VehicleNode *percept_p = (VehicleNode *)userData;
  percept_p->stereo_perception_subscription_success = true;
//    DSTATUS("image info : dataId(%d) seq(%d) timestamp(%d) datatype(%d) index(%d) h(%d) w(%d) dir(%d) "
//            "bpp(%d) bufferlen(%d)", info.dataId, info.sequence, info.timeStamp/1000, info.dataType,
//            info.rawInfo.index, info.rawInfo.height, info.rawInfo.width, info.rawInfo.direction,
//            info.rawInfo.bpp, bufferLen);
//    ROS_INFO("Image time is: %d",info.timeStamp);
  if (imageRawBuffer) {
...
//  #else
    ...
    // img.header.stamp = ros::Time::now();
    img.header.stamp = percept_p->base_time_ + _TICK2ROSTIME(info.timeStamp/1000);
    img.header.seq = info.sequence;
    out_msg.header = img.header;
    out_msg.encoding = sensor_msgs::image_encodings::MONO8;
    out_msg.image = cv_img_stereo;
    sprintf(name, "Image_dataType : %d", info.dataType);
    cv::imshow(name, cv_img_stereo);
    cv::waitKey(1);
    if(info.dataType %2) //publish left
    {
      // cout << "--------Image time: " << img.header.stamp <<endl;
      percept_p->perception_left_publisher.publish(out_msg.toImageMsg());

    }
    else //publish right
      percept_p->perception_right_publisher.publish(out_msg.toImageMsg());
    // img.height = info.rawInfo.height;
      ...
      //   img.data.push_back(imageRawBuffer[i]);

      // img.data.resize(img.height*img.width);
      // img.encoding = "mono8";

      // memcpy(&img.data[0], imageRawBuffer, bufferLen);
      // node_ptr->perception_left_publisher.publish(out_msg.toImageMsg());
    // percept_p->perception_left_publisher.publish(img);

    //   cv::Mat  cv_img_stereo = cv::Mat(480, 640, CV_8U);
    //    memcpy(cv_img_stereo.data, imageRawBuffer, bufferLen);
    //    cv::imshow("test", cv_img_stereo);
    // cv::waitKey(1);
